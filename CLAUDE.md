...
- **Employee photographs** live on whichever disk `config('filesystems.avatars')` names (`AVATAR_DISK` in `.env`), and the column stores a *path*, never a URL. `public` works on one machine only - the database is shared but `storage/` is not, the same reason QR images 404 - and `Avatar.jsx` falls back to initials when the image fails to load. `supabase` (a public bucket, S3-compatible, needs `league/flysystem-aws-s3-v3`) works from every machine and survives the move to a real server. `avatar_path` is hidden and `avatar_url` unappended, for the same reason as the name columns.
- `AvatarStorage` squares and shrinks every upload to 256px with **GD** (there is no ImageMagick on these machines). Phone photos arrive at several megabytes and are drawn at forty pixels.
- **The "FOR" block on the slip is print-only.** BIR Form 2309 has fourteen boxes the RDO or a Chief ticks *by hand* on the hardcopy, so no form asks for it and `completeDetails()` does not write `referred_for`. `config('referral.referred_for')` now feeds only `ReferenceSlipModal`, which prints the empty grid - keep that list matching the paper. The column and its legacy values stay; a stored value prints its box pre-ticked.
